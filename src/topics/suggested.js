    ...
    function getTidsWithSameTags(tid, cutoff) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (cutoff === 0) {
                ...
                const tids = yield database_1.default.getSortedSetRevRange(tags.map(tag => `tag:${tag}:topics`), 0, -1);
            }
            else {
                // The next line calls a function in a module that has not been updated to TS yet
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
                const tids = yield database_1.default.getSortedSetRevRangeByScore(tags.map(tag => `tag:${tag}:topics`), 0, -1, '+inf', Date.now() - cutoff);
            }
            const tidsFiltered = tids.filter(_tid => _tid !== tid); // remove self
            return lodash_1.default.shuffle(lodash_1.default.uniq(tidsFiltered)).slice(0, 10).map(Number);
        });
    }
